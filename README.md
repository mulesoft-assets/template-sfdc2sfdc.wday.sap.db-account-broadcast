...
This Anypoint template serves as a foundation for setting an online sync of accounts from a Salesforce instance to many destination systems, using the Publish-subscribe pattern. Every time there is a new account or a change in an already existing one, the integration will poll for changes in the Salesforce source Org, publish the changes to a topic and each subscriber will be responsible for updating the accounts in the target systems.
...
The application has four different batch jobs consuming this JMS topic, one for migrating the changes to each target system (the second Salesforce Org, Workday, SAP and the database). During the *Process* stage, each Salesforce account will be matched with an existing account in the target system by Name.
